...
<script>
    function auto_send(val){
      if(navigator.onLine){
        id_soal = '<?=$detail_soal_sb->id?>';
        // jawaban = this.value();
        $.get("<?=base_url()?>regis/auto_save_jawaban/sb/"+id_soal+"/"+val, function(data){
            ...
            <div class="card-body">
              <div class="row col-md-12">
                ...
                <div class="col-md-2 mb-2">
                  <a href="<?=base_url()?>regis/soal_sb/<?=$slug?>/<?=$row->id?>/<?=$no_soal?>" class="btn btn-sm <?=($row->jawaban == '0') ? "btn-secondary" : "btn-dark"?> text-white <?=($row->id == $detail_soal_sb->id) ? "active" : ""?>">
                    <h5><?=$no_soal?></h5>
                  ...
                  <div class="col-md-12 pl-5">
                    <div class="row">
                      <div class="icheck-success d-inline col-md-6">
                        <input type="radio" <?=($jawaban['jawaban'] == 'b') ? 'checked' : ''?> id="<?=$detail_soal_sb->id?>_sb_b" name="soal_sb_<?=$detail_soal_sb->id?>" onclick="auto_send('b');" value="b" required>
                        <label style="font-weight: 500;" for="<?=$detail_soal_sb->id?>_sb_b">Benar</label>
                      </div>
                      <div class="icheck-success d-inline col-md-6">
                        <input type="radio" <?=($jawaban['jawaban'] == 's') ? 'checked' : ''?> id="<?=$detail_soal_sb->id?>_sb_s" name="soal_sb_<?=$detail_soal_sb->id?>" onclick="auto_send('s');" value="s" required>
