...
<script>
    function auto_send(val){
      if(navigator.onLine){
        id_soal = '<?=$detail_soal_pg->id?>';
        // jawaban = this.value();
        $.get("<?=base_url()?>regis/auto_save_jawaban/pg/"+id_soal+"/"+val, function(data){
          ...
          <div class="card">
            <div class="card-body">
              <div class="row col-md-12">
                ...
                <div class="col-md-2 mb-2">
                  <a href="<?=base_url()?>regis/soal_pg/<?=$slug?>/<?=$row->id?>/<?=$no_soal?>" class="btn btn-sm <?=($row->jawaban == '0') ? "btn-secondary" : "btn-dark"?> text-white <?=($row->id == $detail_soal_pg->id) ? "active" : ""?>">
                    <h5><?=$no_soal?></h5>
                    <i class="fa fa-check-circle fa-2x"></i>
                  </a>
                </div>
                ...
                <div class="col-md-2 mb-2">
                  <a href="<?=base_url()?>regis/soal_pgsk/<?=$slug?>/<?=$row->id?>/<?=$no_soal?>" class="btn btn-sm <?=($row->jawaban == '0') ? "btn-secondary" : "btn-dark"?> text-white">
                    <h5><?=$no_soal?></h5>
                    <i class="fa fa-check-circle fa-2x"></i>
                  </a>
                </div>
          ...
          <div class="row">
            <div class="col-md-12">
              <div class="card h-100 h5">
                ...
                <div class="card-body">
                  <ol start="<?=$start_no?>">
                    <li class="text-justify"><?=$detail_soal_pg->soal_ujian?></li>
                  </ol>

                  <div class="col-md-12 pl-5">
                    <div class="row">
                      <div class="icheck-success d-inline col-md-6">
                        <input type="radio" <?=($jawaban['jawaban'] == 'a') ? 'checked' : ''?> id="<?=$detail_soal_pg->id?>_pg_a" name="soal_pg_<?=$detail_soal_pg->id?>" onclick="auto_send('a');" value="a" required>
                        <label style="font-weight: 500;" for="<?=$detail_soal_pg->id?>_pg_a">a. <?=$detail_soal_pg->jawaban_a?></label>
                      </div>
                      <div class="icheck-success d-inline col-md-6">
                        <input type="radio" <?=($jawaban['jawaban'] == 'b') ? 'checked' : ''?> id="<?=$detail_soal_pg->id?>_pg_b" name="soal_pg_<?=$detail_soal_pg->id?>" onclick="auto_send('b');" value="b" required>
                        <label style="font-weight: 500;" for="<?=$detail_soal_pg->id?>_pg_b">b. <?=$detail_soal_pg->jawaban_b?></label>
                      </div>
                      <div class="icheck-success d-inline col-md-6">
                        <input type="radio" <?=($jawaban['jawaban'] == 'c') ? 'checked' : ''?> id="<?=$detail_soal_pg->id?>_pg_c" name="soal_pg_<?=$detail_soal_pg->id?>" onclick="auto_send('c');" value="c" required>
                        <label style="font-weight: 500;" for="<?=$detail_soal_pg->id?>_pg_c">c. <?=$detail_soal_pg->jawaban_c?></label>
                      </div>
                      <div class="icheck-success d-inline col-md-6">
                        <input type="radio" <?=($jawaban['jawaban'] == 'd') ? 'checked' : ''?> id="<?=$detail_soal_pg->id?>_pg_d" name="soal_pg_<?=$detail_soal_pg->id?>" onclick="auto_send('d');" value="d" required>
                        <label style="font-weight: 500;" for="<?=$detail_soal_pg->id?>_pg_d">d. <?=$detail_soal_pg->jawaban_d?></label>
                      </div>
                    </div>
                  </div>
                </div>
